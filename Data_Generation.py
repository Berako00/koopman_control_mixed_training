...
def generate_data_forced(x1range, x2range, numICs, mu, lam, T_step, dt, seed):
   # Set random seed for reproducibility
    ...
    u = torch.rand(numICs, T_step) - 0.5
...
def DataGenerator(x1range, x2range, numICs, mu, lam, T, dt):
    ...
    seed = 1
    test_tensor = generate_data_forced(x1range, x2range, round(0.1 * numICs), mu, lam, T, dt, seed)

    seed = 2
    val_tensor = generate_data_forced(x1range, x2range, round(0.1 * numICs), mu, lam, T, dt, seed)

    seed = 3
    train_tensor_unforced = generate_data_unforced(x1range, x2range, round(0.4 * numICs), mu, lam, T, dt, seed)

    seed = 4
    train_tensor_forced = generate_data_forced(x1range, x2range, round(0.4 * numICs), mu, lam, T, dt, seed)
